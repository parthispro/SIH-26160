...
Smart India Hackathon 2026 — Problem Statement SIH26160 Organization: National Technical Research Organisation (NTRO) Category: Software | Theme: Blockchain & Cybersecurity
...
Parses the cleartext IKE (Internet Key Exchange) negotiation packets to extract the negotiated encryption algorithm, Diffie-Hellman group, and whether PFS was used.
Scores the configuration against known security guidelines (NIST SP 800-77 Rev.1), producing a 0–100 risk score with specific findings.
Flags classical (non-quantum-resistant) DH groups as a "Harvest-Now-Decrypt-Later" (HNDL) risk — traffic encrypted today with weak key exchange could be recorded now and decrypted later once quantum computers mature.
Generates a corrected ipsec.conf snippet using secure parameter choices, so a misconfiguration can be fixed immediately.
Presents all of this through a simple interactive dashboard.
